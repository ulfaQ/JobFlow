...
#       pdf_name = "output_" + datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S") + ".pdf"
        pdf_name = "output-" + "TEST" + ".pdf"

        save_name = os.path.join("output/", pdf_name)
    ...
    def _generate_job_info(self, customer, \
        ...
        if self.line < 130:
            self.c.drawString(130, self.line - 15, "More jobs on the next page --->")
            self.line = 790
            self.c.showPage()
